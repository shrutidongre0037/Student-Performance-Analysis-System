<?php
    ...
    $query="select asub_id,asub_date,asub_file,asub_status,s_name,as_topic,sub.s_id from tm_submit sub inner join tm_student s on s.s_id=sub.s_id inner join tm_assignment a on a.as_id=sub.as_id where asub_date like '".$date."%' and sub.as_id in(select as_id from tm_assignment where st_id=$stid)"; 
    $rssub_assign=mysqli_query($con,$query);
?>
<!DOCTYPE html>
                                                    ...
                                                    <div class="table-responsive">
                                                        <table class="table table-striped table-bordered table-hover" id="dtuser">
                                                            ...
                                                            <tbody>
                                                              ...
                                                              { 
                                                                  $cnt=1;
                                                                  while($row=mysqli_fetch_row($rssub_assign))
                                                                  {
                                                                    echo"<tr>";
                                                                    echo"<td>".$cnt++."</td>";
                                                                    echo"<td>".$row[4]."</td>";
                                                                    echo"<td>";
                                                                    $sql="select course_name from tm_course where course_id=(select course_id from tm_student where s_id=$row[6])";
                                                                    $rscrse=mysqli_query($con,$sql);
                                                                    $cnm=mysqli_fetch_row($rscrse);
                                                                    echo $cnm[0];
                                                                    echo"</td>";
                                                                    echo"<td>";
                                                                    $sql="select sem_name from tm_semester where sem_id=(select sem_id from tm_student where s_id=$row[6])";
                                                                    ...
                                                                    echo"<td>".$row[5]."</td>";
                                                                    echo"<td>".$row[1]."</td>";
                                                                    echo"<td><a class='btn btn-primary btn-sm' href='dbevaluateassign.php?fname=".$row[2]."'><i class='icon-download-alt' style='color:white;'></i></a></td>";
                                                                    echo "<td>";
                                                                    if($row[3]==0)
                                                                    ...
                                                                    echo "</td>";
                                                                   //echo"<td><a id='btn_check'>Click</a></td>";
                                                                    echo"<td><a class='btn btn-warning btn-md' href='dbevaluateassign.php?asubid=".$row[0]."' id='btn_check' >Check</a></td>";
                                                                    echo"</tr>";
                                                                    $cnt=$cnt++;
